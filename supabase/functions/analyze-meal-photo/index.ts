...
const analyzeWithOpenAi = async (photoUrl: string, locale: string): Promise<AiMealAnalysis> => {
        ...
        content: [
          {
            type: 'text',
            text: [
              'Analyze the meal photo for the Nutora app.',
              ...
              'Estimate the weight of each visible ingredient and calculate nutritional values based on official tables.',
...
serve(async (request) => {
  ...
  let body: AnalyzeMealPhotoRequest;
  ...
  if (!body.photoUrl) return json({ error: 'photoUrl is required' }, 400);
  ...
  try {
    ...
    await logAnalysis({
      ...
      requestPayload: { photoUrl: body.photoUrl },
    ...
    return json({ error: 'Nie udało się przeanalizować zdjęcia posiłku.' }, 500);
  }
});
